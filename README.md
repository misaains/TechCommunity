...
### Бэкенд на Python (Flask/FastAPI)

**API Base URL:** `http://172.20.10.2:8000/api`  
**Фронтенд URL:** `http://172.20.10.6:8000`

### Все запросы используют fetch API

Все HTTP-запросы в `api.js` выполняются через нативный `fetch()`. Примеры чистого fetch (без обёрток) смотрите в файле **`FETCH_EXAMPLES.md`**.

### Настройка CORS на Python бэкенде
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://172.20.10.6:8000", "*"],
...
**Документация:**
- `API_SPECIFICATION.md` - спецификация всех эндпоинтов для бэкенда
- `FETCH_EXAMPLES.md` - примеры чистых fetch-запросов
- `TESTING_CHECKLIST.md` - полный чек-лист тестирования
- `API_USAGE_EXAMPLES.md` - примеры использования через window.API
